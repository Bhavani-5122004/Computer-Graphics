    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                si = siIntermediate;
                ray.t = si.t;
                int index = this->getIdx(i + node.firstPrim);
                
                si.triangle.uv1 = this->tris[index].uv1;
                si.triangle.uv2 = this->tris[index].uv2;
                si.triangle.uv3 = this->tris[index].uv3;
                si.triangle.v1 = this->tris[index].v1;
                si.triangle.v2 = this->tris[index].v2;
                si.triangle.v3 = this->tris[index].v3;
                si.triangle.normal = this->tris[index].normal;
                si.triangle.centroid = this->tris[index].centroid;
